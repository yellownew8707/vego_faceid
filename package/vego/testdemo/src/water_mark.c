...
void argb2yuv420sp(unsigned char* src_p, unsigned char* alph, unsigned int width, unsigned int height, unsigned char* dest_y,
...
void yuv420sp_blending(unsigned int bg_width, unsigned int bg_height, unsigned int left, unsigned int top, unsigned int fg_width,
	unsigned int fg_height, unsigned char *bg_y, unsigned char *bg_c, unsigned char *fg_y, unsigned char *fg_c, unsigned char *alph) {
...
int region_bright_or_dark(unsigned int bg_width, unsigned int bg_height, unsigned int left, unsigned int top, unsigned int fg_width,
		unsigned int fg_height, unsigned char *bg_y) {
	unsigned char *bg_y_p = NULL;
...
void yuv420sp_blending_adjust_brightness(unsigned int bg_width, unsigned int bg_height, unsigned int left, unsigned int top,
		unsigned int fg_width, unsigned int fg_height, unsigned char *bg_y, unsigned char *bg_c, unsigned char *fg_y, unsigned char *fg_c,
		unsigned char *alph) {
	unsigned char *bg_y_p = NULL;
	unsigned char *bg_c_p = NULL;
	int is_brightness = 0;
	...
	is_brightness = 0;/*region_bright_or_dark(bg_width,bg_height,left,top,
	 fg_width,fg_height,bg_y);*/

//	LOGD("start bg_y_p:0x%x ,bg_y:0x%x  ,bg_c_p:0x%x ,bg_c:0x%x ,left:%d ,top:%d ,fg_y:0x%x ,fg_c:0x%x",bg_y_p,bg_y,bg_c_p,bg_c,left,top,fg_y,fg_c);
	bg_y_p = bg_y + top * bg_width + left;
	bg_c_p = bg_c + (top >> 1) * bg_width + left;
//	LOGD("========= yuv420sp_blending_adjust_brightness start fg_height:%d ,fg_width:%d =========",fg_height,fg_width);
	if (is_brightness) {
		for (i = 0; i < (int) fg_height; i++) {
			if ((i & 1) == 0) {
				for (j = 0; j < (int) fg_width; j++) {
...
//	LOGD("========= yuv420sp_blending_adjust_brightness end=========");
}
...
int watermark_blending(BackGroudLayerInfo *bg_info, WaterMarkInfo *wm_info, ShowWaterMarkParam *wm_Param) {
	...
	for (i = 0; i < (int) wm_Param->number; i++) {
		...
		yuv420sp_blending(bg_info->width, bg_info->height, (wm_Param->pos.x + wm_info->width * i), wm_Param->pos.y, wm_info->width,
				wm_info->height, bg_info->y, bg_info->c, wm_info->single_pic[id].y, wm_info->single_pic[id].c,
...
int watermark_blending_ajust_brightness(BackGroudLayerInfo *bg_info, WaterMarkInfo *wm_info, ShowWaterMarkParam *wm_Param) {
	...
	int i;
	int id;
//	LOGD("========= watermark_blending_ajust_brightness start=========");
	if (wm_info->width * wm_Param->number > bg_info->width) {
		printf("watermark_blending error region\n");
		return -1;
	}
//	LOGD("========= watermark_blending_ajust_brightness number:%d =========",wm_Param->number);
	for (i = 0; i < wm_Param->number; i++) {
		id = wm_Param->id_list[i];
//		LOGD("========= watermark_blending_ajust_brightness id:%d =========",id);
		yuv420sp_blending_adjust_brightness(bg_info->width, bg_info->height, (wm_Param->pos.x + wm_info->width * i), wm_Param->pos.y,
				wm_info->width, wm_info->height, bg_info->y, bg_info->c, wm_info->single_pic[id].y, wm_info->single_pic[id].c,
				wm_info->single_pic[id].alph);
	}
	return 0;
}
...
//		LOGD("===== i:%d w,h :%d,%d ,y:0x%x ,alph:0x%x ,c:0x%x",
//				i,
//				waterMark->srcInfo.width,
//				waterMark->srcInfo.height,
//				waterMark->srcInfo.single_pic[i].y,
//				waterMark->srcInfo.single_pic[i].alph,
//				waterMark->srcInfo.single_pic[i].c);
		if (tmp_argb == NULL) {
		...
		argb2yuv420sp(tmp_argb, waterMark->srcInfo.single_pic[i].alph, width, height, waterMark->srcInfo.single_pic[i].y,
				waterMark->srcInfo.single_pic[i].c);
...
void waterMarkShowTime(WaterMark* waterMark, int x, int y, int st) {
	...
	int timestamp[4];
	/* get current time */
//	LOGD("========= waterMarkShowTime start=========");
	time(&now);
	/*
    memset(&param, 0, sizeof(ShowWaterMarkParam));
	data_convert(st, 4, (int *) &timestamp);
	param.pos.x = x;
	param.pos.y = y;
	param.id_list[0] = timestamp[0];
	param.id_list[1] = timestamp[1];
	param.id_list[2] = timestamp[2];
	param.id_list[3] = timestamp[3];
	
	param.number = 4;
	printf("waterMark init ok\n");
	*/
