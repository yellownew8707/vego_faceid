...
PLATFORM_CHOICES=(arbutus banana cherry durian emblica)
...
VARIANT_CHOICES=(vego dragonboard)
...
function pack() {
	...
	if [ "x$board_platform" = "xunclear" ]; then
		chip=sun5i
	elif [ "x$board_platform" = "xarbutus" ]; then
		chip=sun8iw5p1
	elif [ "x$board_platform" = "xemblica" ]; then
		chip=sun8iw11p1
	elif [ "x$board_platform" = "xbanana" ]; then
		chip=sun8iw6p1
	elif [ "x$board_platform" = "xdurian" ]; then
		chip=sun50iw1p1
	else
...
function cdevice()
{
    T=$(gettop)
    if [ "$T" ]; then
	    \cd $(gettop)/target/vego/${TARGET_BOARD}
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
    fi
}
...
function cconfigs()
{
    T=$(gettop)
    if [ "$T" ]; then
	    \cd $(gettop)/target/vego/${TARGET_BOARD}/configs
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
    fi
}
