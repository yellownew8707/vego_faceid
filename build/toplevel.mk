...
prepare-tmpinfo: FORCE
	...
	$(_SINGLE)$(NO_TRACE_MAKE) -j1 -r -s -f build/scan.mk SCAN_TARGET="targetinfo" SCAN_DIR="target/vego" SCAN_NAME="target" SCAN_DEPS="*.mk $(TOPDIR)/build/kernel*.mk $(TOPDIR)/build/target.mk" SCAN_DEPTH=2 SCAN_EXTRA="" SCAN_MAKEOPTS="TARGET_BUILD=1"
...
.config: ./scripts/config/conf $(if $(CONFIG_HAVE_DOT_CONFIG),,prepare-tmpinfo)
	@+if [ \! -e .config ] || ! grep CONFIG_HAVE_DOT_CONFIG .config >/dev/null; then \
		if [ "x$(TARGET_BUILD_VARIANT)" = "xvego" ]; then \
			[ -e target/vego/$(TARGET_BOARD)/defconfig ] && \
			cp target/vego/$(TARGET_BOARD)/defconfig .config; \
		elif [ "x$(TARGET_BUILD_VARIANT)" = "xdragonboard" ]; then \
			[ -e target/vego/$(TARGET_BOARD)/defconfig-db ] && \
			cp target/vego/$(TARGET_BOARD)/defconfig-db .config; \
		fi \
	elif [ -e .config -a "x$(TARGET_BUILD_VARIANT)" = "xvego" ]; then \
		cp target/vego/$(TARGET_BOARD)/defconfig .config; \
...
menuconfig: scripts/config/mconf prepare-tmpinfo FORCE
	if [ -z $(TARGET_BOARD) ]; then \
		...
		if cat .config|grep CONFIG_TARGET_BOARD=\"$(TARGET_BOARD)\" >> /dev/null; then \
			[ "x$(TARGET_BUILD_VARIANT)" = "xvego" ] && \
			cp .config target/vego/$(TARGET_BOARD)/defconfig; \
			[ "x$(TARGET_BUILD_VARIANT)" = "xdragonboard" ] && \
			cp .config target/vego/$(TARGET_BOARD)/defconfig-db; \
			echo ; \
		else \
			rm .config; \
		fi \
...
kernel_oldconfig: prepare_kernel_conf
	$(_SINGLE)$(NO_TRACE_MAKE) -C target/vego oldconfig

kernel_menuconfig: prepare_kernel_conf
	$(_SINGLE)$(NO_TRACE_MAKE) -C target/vego menuconfig

kernel_nconfig: prepare_kernel_conf
	$(_SINGLE)$(NO_TRACE_MAKE) -C target/vego nconfig
