...
#
# c. Switch uart port
#    - Need to add your chip configs into ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_pin
...
if [ "x${PACK_CHIP}" = "xsun5i" ]; then
	...
	PACK_BOARD_PLATFORM=arbutus
	ARCH=arm
elif [ "x${PACK_CHIP}" = "xsun8iw6p1" ]; then
	PACK_BOARD_PLATFORM=banana
	ARCH=arm
elif [ "x${PACK_CHIP}" = "xsun8iw11p1" ]; then
	PACK_BOARD_PLATFORM=emblica
	ARCH=arm
elif [ "x${PACK_CHIP}" = "xsun50iw1p1" ]; then
	PACK_BOARD_PLATFORM=durian
	ARCH=arm64
else
...
function uart_switch()
{
	...
	TX=`awk  '$0~"'$PACK_CHIP'"{print $2}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_pin`
	RX=`awk  '$0~"'$PACK_CHIP'"{print $3}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_pin`
	PORT=`awk  '$0~"'$PACK_CHIP'"{print $4}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_pin`
	MS=`awk  '$0~"'$PACK_CHIP'"{print $5}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_pin`
	CK=`awk  '$0~"'$PACK_CHIP'"{print $6}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_pin`
	DO=`awk  '$0~"'$PACK_CHIP'"{print $7}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_pin`
	DI=`awk  '$0~"'$PACK_CHIP'"{print $8}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_pin`

	BOOT_UART_ST=`awk  '$0~"'$PACK_CHIP'"{print $2}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	BOOT_PORT_ST=`awk  '$0~"'$PACK_CHIP'"{print $3}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	BOOT_TX_ST=`awk  '$0~"'$PACK_CHIP'"{print $4}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	BOOT_RX_ST=`awk  '$0~"'$PACK_CHIP'"{print $5}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	UART0_ST=`awk  '$0~"'$PACK_CHIP'"{print $6}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	UART0_USED_ST=`awk  '$0~"'$PACK_CHIP'"{print $7}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	UART0_PORT_ST=`awk  '$0~"'$PACK_CHIP'"{print $8}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	UART0_TX_ST=`awk  '$0~"'$PACK_CHIP'"{print $9}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	UART0_RX_ST=`awk  '$0~"'$PACK_CHIP'"{print $10}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	UART1_ST=`awk  '$0~"'$PACK_CHIP'"{print $11}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	JTAG_ST=`awk  '$0~"'$PACK_CHIP'"{print $12}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	MS_ST=`awk  '$0~"'$PACK_CHIP'"{print $13}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	CK_ST=`awk  '$0~"'$PACK_CHIP'"{print $14}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	DO_ST=`awk  '$0~"'$PACK_CHIP'"{print $15}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	DI_ST=`awk  '$0~"'$PACK_CHIP'"{print $16}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	MMC0_ST=`awk  '$0~"'$PACK_CHIP'"{print $17}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
	MMC0_USED_ST=`awk  '$0~"'$PACK_CHIP'"{print $18}' ${PACK_TOPDIR}/target/vego/generic/debug/card_debug_string`
...
function do_prepare()
{
	...
	if [ ! -d ${PACK_TOPDIR}/target/vego/${PACK_BOARD}/configs ] ; then
		pack_error "Board's directory \
			\"${PACK_TOPDIR}/target/vego/${PACK_BOARD}/configs\" is not exist."
		show_boards
		exit 1
	fi
	...
	for file in ${tools_file_list[@]} ; do
		cp -f ${PACK_TOPDIR}/target/vego/$file ${ROOT_DIR}/image/ 2> /dev/null
	done
	...
	cp -f ${PACK_TOPDIR}/target/vego/generic/configs/env-${ENV_SUFFIX}.cfg ${ROOT_DIR}/image/env.cfg 2> /dev/null
	...
	for file in ${boot_resource_list[@]} ; do
		cp -rf ${PACK_TOPDIR}/target/vego/`echo $file | awk -F: '{print $1}'` \
			${ROOT_DIR}/`echo $file | awk -F: '{print $2}'` 2>/dev/null
	done

	printf "copying boot file\n"
	for file in ${boot_file_list[@]} ; do
		cp -f ${PACK_TOPDIR}/target/vego/`echo $file | awk -F: '{print $1}'` \
			${ROOT_DIR}/`echo $file | awk -F: '{print $2}'` 2>/dev/null
	done
...
function do_signature()
{
	...
	if [ "x${PACK_SIG}" = "xprev_refurbish" ] ; then
		if [ "x${ARCH}" = "xarm64" ] ; then
			cp -v ${PACK_TOPDIR}/target/vego/generic/sign_config/dragon_toc_a64_no_secureos.cfg dragon_toc.cfg
		else
			cp -v ${PACK_TOPDIR}/target/vego/generic/sign_config/dragon_toc_no_secureos.cfg dragon_toc.cfg
		fi
	else
		if [ "x${ARCH}" = "xarm64" ] ; then
			if [ "x${PACK_PLATFORM}" = "xdragonboard" ] ; then
				cp -v ${PACK_TOPDIR}/target/vego/generic/sign_config/dragon_toc_a64_linux.cfg dragon_toc.cfg
			else
				cp -v ${PACK_TOPDIR}/target/vego/generic/sign_config/dragon_toc_a64.cfg dragon_toc.cfg
			fi
		else
			if [ "x${PACK_PLATFORM}" = "xlinux" ] ; then
				cp -v ${PACK_TOPDIR}/target/vego/generic/sign_config/dragon_toc.cfg dragon_toc.cfg
			elif [ "x${PACK_PLATFORM}" = "xtina" ] ; then
				cp -v ${PACK_TOPDIR}/target/vego/generic/sign_config/dragon_toc.cfg dragon_toc.cfg
			elif [ "x${PACK_PLATFORM}" = "xandroid" ] ; then
				cp -v ${PACK_TOPDIR}/target/vego/generic/sign_config/dragon_toc.cfg dragon_toc.cfg
			elif [ "x${PACK_PLATFORM}" = "xdragonboard" ] ; then
				cp -v ${PACK_TOPDIR}/target/vego/generic/sign_config/dragon_toc_linux.cfg dragon_toc.cfg
			fi
		fi
	fi
	...
	dragonsecboot -toc1 dragon_toc.cfg ${ROOT_DIR}/keys ${PACK_TOPDIR}/target/vego/generic/sign_config/cnf_base.cnf
	if [ $? -ne 0 ]
	then
...
function do_pack_vego()
{
	printf "packing for vego linux\n"
